...
// ─── الصف الموحّد (نفس شكل PrintTypeSection) ─────────────────────────────────
function Row({
  ...
  return (
    <div
      className="flex items-center gap-2 bg-white p-1 border border-gray-200 rounded-md shadow-sm"
      style={{ direction: "rtl" }}
    >
      {/* زر يمين */}
      <button
      ...
      {/* حقول وسط */}
      <div className="flex-1 flex items-center gap-2 overflow-hidden min-h-[40px]">
        {children && (
...
function FSelect({ label, value, onChange, options }: {
  ...
  return (
    <div className="relative min-w-[150px] flex-1 max-w-[230px]">
...
// ─── فاصل القسم ───────────────────────────────────────────────────────────────
// function SectionLabel({ title, color = "#111" }: { title: string; color?: string }) {
//   return (
...
export function StickerDetailsSection(props: StickerDetailsSectionProps) {
  ...
  // ── حالات التفعيل المحلية — كلها مقفلة بالبداية ──
  ...
  const [rollDirEnabled,  setRollDirEnabled]  = useState(false);
  ...
  return (
    <div className="bg-white rounded-2xl p-6 md:p-8 shadow-sm border border-neutral-200" style={{ direction: "rtl" }}>
      ...
      <div className="space-y-7">
        ...
        <div>
          ...
          <div className="grid grid-cols-2 md:grid-cols-5 gap-3">
            {[
              ...
              <div key={f.label}>
                <label className="block text-xs font-semibold text-neutral-600 mb-2">{f.label}</label>
                <input
                  type="number"
        ...
        <div className="bg-blue-50 border border-blue-200 rounded-xl p-5">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            <div>
              <label className="block text-xs font-semibold text-neutral-700 mb-2">عرض الرول المتاح (مم)</label>
              <input
                type="number"
                placeholder="330"
                value={rollWidth}
            ...
            {requiredRollWidthInfo && (
              <>
                <div>
                  <label className="block text-xs font-semibold text-neutral-700 mb-2">عرض الرول المطلوب (مم)</label>
                ...
                <div>
                  <label className="block text-xs font-semibold text-neutral-700 mb-2">عدد الملصقات في العرض</label>
                  ...
                  <p className="text-[10px] text-neutral-600 mt-1">ملصق</p>
                </div>
              </>
            )}
          </div>
            ...
            <div className="mt-4 bg-red-100 border border-red-300 rounded-lg p-3 flex items-start gap-2">
              ...
              <div>
                ...
                <p className="text-xs text-red-700 mt-1">
                  ...
                  <br />يرجى: تقليل عرض الملصق، تقليل الجاب، أو استخدام رول أعرض.
                </p>
              </div>
            </div>
          )}
        ...
        {/* ══════════════════════════════════════════
            باقي الخصائص — صفوف مقفلة بالبداية
        ══════════════════════════════════════════ */}
        <div className="space-y-1.5">
          ...
          {/* الطباعة */}
          <div className="bg-gray-50 rounded-md border border-gray-200 p-1">
            <PrintTypeSection />
          </div>

          {/* التصاميم — مقفل بالبداية */}
          <Row
            ...
            priceSlot={
              materialEnabled ? (
                <div className="flex items-center gap-1.5">
                  <span className="font-bold">{materialPrice} ر.س</span>
                  <ChangeChoosesPrice listTypeOfData={materialsData} />
                </div>
              ) : <span className="text-gray-300">0.00 ر.س</span>
            }
          >
            {materialEnabled && (
          ...
          <Row
            ...
            priceSlot={
              <div className="flex items-center gap-1.5">
                ...
                <span>{hasLamination ? `+${PRICING_CONFIG.additionalCosts.lamination.perSqm} ر.س` : "0.00 ر.س"}</span>
              </div>
            }
          >
            {hasLamination && (
          ...
          <Row
            ...
            priceSlot={
              <div className="flex items-center gap-1.5">
                ...
                <span>{hasSilkScreen ? `+${PRICING_CONFIG.additionalCosts.silkScreen.perSqm} ر.س` : "0.00 ر.س"}</span>
              </div>
            }
          />
          ...
          <Row
            ...
            priceSlot={
              <div className="flex items-center gap-1.5">
                ...
                <span>{hasEmbossing ? `${embossingColors.find(c => c.value === embossingColor)?.price || 0} ر.س` : "0.00 ر.س"}</span>
              </div>
            }
          >
            {hasEmbossing && (
          ...
          <Row
            ...
            priceSlot={
              <div className="flex items-center gap-1.5">
                ...
                <span>{hasSpotUV ? `+${PRICING_CONFIG.additionalCosts.spotUV.perSqm} ر.س` : "0.00 ر.س"}</span>
              </div>
            }
          >
            {hasSpotUV && (
          ...
          {/* التركيب — مقفل بالبداية */}
          <Row
            label="التركيب" subLabel="Assembly"
          ...
          {/* اتجاه الرول — مقفل بالبداية */}
          <Row
            label="اتجاه الرول" subLabel="Roll Direction"
            isActive={rollDirEnabled} onToggle={() => setRollDirEnabled(!rollDirEnabled)}
            priceSlot={<span className="text-gray-400 text-xs">—</span>}
          >
            {rollDirEnabled && (
              <div className="flex items-center gap-1.5 py-1 flex-wrap">
                {[1, 2, 3, 4, 5, 6, 7, 8].map((num) => (
                  <button
                    ...
                    className={`w-10 h-10 overflow-hidden rounded border-2 transition-all
